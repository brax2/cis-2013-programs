...
var passwordCheckOld = function () 
{
    ...
    stringOutput = "";
 //This ensures that the password is greater than 8 characters, otherwise it returns "This password is way too short."
   if (stringPassword.length<8)
   {
      stringOutput = "This password is way too short.";
   }
   //If the password is indeed longer than 8 characters, we then check to make sure it is not one of the bad passwords
   else
   {
   
      for (var f=0; f < arrayBadPasswords.length; f++)
      {
         //If the password shows up on the bad password array, it returns the message "This is a horrible password..." in the form of a string output
         if (stringPassword === arrayBadPasswords[f])
         {
            stringOutput = "This is a horrible password...";
         }
      }
   }
   //In the case that the password triggers the IF statements, and the output is not equal to " ", then the string output is given
   if (stringOutput != "")
   {
      $("output").value= stringOutput;
   }
   //If the password has not triggered the IF statements, then the password is successful and states "This is a great password!"
   else
   {
      $("output").value= "This is a great password!";
   }
};
